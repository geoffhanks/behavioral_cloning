...
""" Dataset for behavioural cloning model with nullspace and path following"""
...
""" Dataset for behavioural cloning model with only the null space configuration """  
class NullSpaceOnlyDataset(Dataset):
    """
    Custom pytorch dataloader for MLP
    """
    ...
    def __getitem__(self, idx):
        ...
        image = (read_image(img_path))/255 # divide by 255 to transform uint8 to a float between 0 and 1
        joint_data = torch.tensor(self.data.iloc[idx, 2:9].values)
        label = torch.tensor(self.data.iloc[idx, 16:].values)
...
""" test the functionality of the dataset in the dataloader"""
def main():
    # test the data loader by displaying images and their labels in the windo title
    train_dataset = NullSpaceOnlyDataset("./Data/nullspace_and_stiffness_BC/trainfile9.csv", 
